...
def build_directions(df_found: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df_found.iterrows():
        ...
        rede_fim_de_semana = pd.notna(data_rede) and data_rede.weekday() >= 5
        aut_fim_de_semana = pd.notna(data_aut) and data_aut.weekday() >= 5

        if (
            not pd.isna(diff_data)
            and int(diff_data) != 0
            and not rede_fim_de_semana
            and not aut_fim_de_semana
        ):
            acoes.append(
                f"Alterar data no AUTCOM para {format_date(row['Data REDE'])}, para ficar igual à REDE"
            )

        if not pd.isna(diff_bru) and abs(float(diff_bru)) > 0.01:
            acoes.append(
                f"Alterar valor bruto no AUTCOM para {brl(row['Valor bruto REDE'])}, para ficar igual à REDE"
            )

        if not pd.isna(diff_liq) and abs(float(diff_liq)) > 0.01:
            acoes.append(
                f"Alterar valor líquido no AUTCOM para {brl(row['Valor líquido REDE'])}, para ficar igual à REDE"
            )
